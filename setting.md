...
设*n*为谱面Note总数，$ a_i $为第i个Note的击打准确率，则：
...
- 连击得分
  设*p*、*q*为累计倍率：
  - 每50连击，*p*增加0.5%，且若连击归零，*p*变为0；
  - 每连续得到100个Perfect评价，*q*增加0.5%，且若得到Great及以下的判定，*q*变为0。
设每个Note的得分为$ x_i $，则：
$$
x_i=a_i(p+q)
$$
计算分为两部分：
- 实际总分：假定所有的Note判定均为Perfect时，累加的结果。
- 实际得分：玩家游玩时Note得分累加的结果。
则得分为：
$$
/frac{实际得分}{实际总分}*1000000
$$
...
按键的判定范围为Note所在轨道的左、右各一个轨道之间（共计三个轨道的宽度）。
下表为各种按键的判定范围（单位：每$ /frac{1}{60} $秒）以及Miss判定的情况：

| Note种类                  | Perfect | Great  | Good   | Bad    | Miss                                 |
| ------------------------- | ------- | ------ | ------ | ------ | ------------------------------------ |
| 单键                      | -3 ~ +3 | -6 ~ +6 | -7 ~ +7 | -8 ~ +8 | 无输入 |
| 滑键                      | -3 ~ +3 | -6 ~ +6 | -7 ~ +7 | -8 ~ +8 | 无输入 |
| 直长条头                  | -3 ~ +3 | -6 ~ +6 | -7 ~ +7 | -8 ~ +8 | 无输入或滑动 |
| 直长条尾                  | -3 ~ +3 | -6 ~ +6 | -7 ~ +7 | -8 ~ +8 | 无输入或滑动 |
| 直长条尾滑键              | -4 ~ +4 | -7 ~ +7 | -8 ~ +8 | -9 ~ +9 | 无输入或在击打长条时直接松开滑条 |
| 斜长条头                  | -4 ~ +4 | -7 ~ +7 | -8 ~ +8 | -9 ~ +9 | 无输入或滑动 |
| 斜长条尾                  | -4 ~ +4 | -7 ~ +7 | -8 ~ +8 | -9 ~ +9 | 无输入或滑动 |
| 斜长条尾滑键              | 0 ~ +6  | 无      | 无      | 无      | 无输入或在击打长条时直接松开滑条 |
| 滑条节点                  | 0 ~ +12 | 无      | 无      | 无      | 无输入 |
