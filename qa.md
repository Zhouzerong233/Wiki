...
获取auth key请在内测群使用`注册内测`指令后，再使用`获取密钥`指令，机器人会将密钥通过私聊的形式发送给你

auth key遗忘或丢失请使用`查询密钥`指令查询当前所有的密钥

更换设备或更新游戏后，如果原有auth key无法使用，请先使用`查询密钥`查询原有auth key的id，再使用`解绑 <id>`清除设备绑定信息，解绑后即可使用原有key进行登陆。
